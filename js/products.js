...
const PRODUCTS = {
    ...
    ensureDBInitialized: function() {
        if (typeof DB !== 'undefined' && typeof DB.init === 'function') {
            DB.init();
        } else {
            console.error('وحدة قاعدة البيانات غير متوفرة');
            UTILS.showNotification('خطأ في تهيئة قاعدة البيانات', 'error');
        }
    },
    
    safeDBGet: function(key, defaultValue = []) {
        this.ensureDBInitialized();
        return DB.get(key, defaultValue);
    },
    
    safeDBSet: function(key, data) {
        this.ensureDBInitialized();
        return DB.set(key, data);
    },
    
    safeDBAdd: function(key, item) {
        this.ensureDBInitialized();
        return DB.add(key, item);
    },
    
    safeDBUpdate: function(key, id, updates) {
        this.ensureDBInitialized();
        return DB.update(key, id, updates);
    },
    
    safeDBRemove: function(key, id) {
        this.ensureDBInitialized();
    ...
    setupEventHandlers: function() {
        ...
        if (saveBtn) {
            saveBtn.addEventListener('click', this.saveProduct.bind(this));
        }
        
        // تحديث منتج
        const updateBtn = document.getElementById('update-product-btn');
        if (updateBtn) {
            updateBtn.addEventListener('click', this.updateProduct.bind(this));
        }
        ...
        if (importBtn) {
            importBtn.addEventListener('click', this.importProducts.bind(this));
        }
        
        // تصدير المنتجات
        const exportBtn = document.getElementById('export-products-btn');
        if (exportBtn) {
            exportBtn.addEventListener('click', this.exportProducts.bind(this));
        }
        
        // معالجة تحميل الصورة
        const productImageInput = document.getElementById('productImage');
        if (productImageInput) {
            productImageInput.addEventListener('change', this.handleImageUpload.bind(this));
        }
        
        const editProductImageInput = document.getElementById('editProductImage');
        if (editProductImageInput) {
            editProductImageInput.addEventListener('change', this.handleEditImageUpload.bind(this));
        }
    },
    
    loadProducts: function() {
        let products = this.safeDBGet(CONSTANTS.STORAGE_KEYS.PRODUCTS);
    ...
    updateFilterStats: function(filteredProducts) {
        const allProducts = this.safeDBGet(CONSTANTS.STORAGE_KEYS.PRODUCTS);
    ...
    loadSuppliersDropdown: function() {
        const suppliers = this.safeDBGet(CONSTANTS.STORAGE_KEYS.SUPPLIERS);
    ...
    completeSaveProduct: function(productData) {
        ...
        const savedProduct = this.safeDBAdd(CONSTANTS.STORAGE_KEYS.PRODUCTS, newProduct);
    ...
    editProduct: function(productId) {
        const products = this.safeDBGet(CONSTANTS.STORAGE_KEYS.PRODUCTS);
    ...
    updateProduct: function() {
        ...
        const updateProduct = (imageData = null) => {
            ...
            const updatedProduct = this.safeDBUpdate(CONSTANTS.STORAGE_KEYS.PRODUCTS, productId, updates);
    ...
    toggleProductStatus: function(productId) {
        ...
        this.safeDBSet(CONSTANTS.STORAGE_KEYS.PRODUCTS, products);
    ...
    deleteProduct: function(productId) {
        ...
        const sales = this.safeDBGet(CONSTANTS.STORAGE_KEYS.SALES);
        ...
        const success = this.safeDBRemove(CONSTANTS.STORAGE_KEYS.PRODUCTS, productId);
    ...
    executeImport: function(importedProducts, strategy) {
        const existingProducts = this.safeDBGet(CONSTANTS.STORAGE_KEYS.PRODUCTS);
        ...
        this.safeDBSet(CONSTANTS.STORAGE_KEYS.PRODUCTS, newProducts);
    ...
    updateProductQuantity: function(productId, quantityChange) {
        ...
        return this.safeDBSet(CONSTANTS.STORAGE_KEYS.PRODUCTS, products);
    }
};
